...
const AddList= async(req,res,next)=>{
    ...
    try{
        ...
        const lastList = await listSchema.findOne({board:board_id}).sort({order:-1}).select("order");
        let new_order ;
        ...
        const list = new listSchema({name:list_name,owner:id,board:board_id,order:new_order});
        await list.save();
...
const UpdateList = async(req,res,next)=>{
    ...
    try{
        ...
        if(list.owner.toString()!== id) return res.status(400).json({msg: "unauthorized"});
        list.name = list_name;
        await list.save();
...
const DeleteList = async(req,res,next)=>{
    ...
    try{
        ...
        if(list.owner.toString()!== id) return res.status(400).json({msg:"forbidden"});
...
const ChangeOrder = async(req,res,next)=>{
    ...
    if(!list_id || (!LB && !TB)) return res.status(400).json({
